...
def branch_info(label: str) -> dict[str, str]:
    return BRANCHES.get(clean_text(label), BRANCHES["신평"])
...
def course_id_from_href(href: str, fallback: str) -> str:
    ...
    if uid:
        return uid
    return clean_text(fallback) or re.sub(r"\D+", "", href)
        ...
        rows.append(
            {
                ...
                "category": "노인복지관",
                "target": "어르신",
                "raw_url": urljoin(page_url, link["href"]),
                "period": period,
                "schedule_raw": schedule,
    ...
    while queue and len(visited) < max_pages and (limit is None or len(rows) < limit):
        ...
        for link in links:
            if link not in visited and link not in queue:
                queue.append(link)
    return rows, {"pages": len(visited), "detail_pages": detail_pages, "pagination_detected": pagination_detected}
