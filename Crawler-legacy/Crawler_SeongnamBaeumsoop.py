...
def should_skip_course(row: dict[str, Any]) -> bool:
    ...
    return any(pattern in title for pattern in PRACTICE_TITLE_PATTERNS)
...
def parse_course_row(tr: Tag, office: dict[str, str], source_url: str) -> dict[str, Any] | None:
    ...
    for element in tr.select("[onclick*='fn_learning_detail']"):
        ...
        if match:
            ...
            break
    if not title and not learning_id:
        return None
...
def validate_office(s: requests.Session, office: dict[str, str], timeout: int) -> dict[str, Any] | None:
    ...
    if is_waiting_page(html):
        return None
    soup = BeautifulSoup(html, "html.parser")
    rows = [tr for tr in soup.select("table tbody tr") if parse_course_row(tr, office, url)]
    if not rows:
        return None
    first = parse_course_row(rows[0], office, url)
    if first and not office.get("branch"):
        office["branch"] = first.get("branch") or office["office_code"]
    return {"html": html, "soup": soup, "rows": rows, "max_page": max_page_from_soup(soup)}
    ...
    *,
    ...
    for office in candidates:
        ...
        if not validation:
            continue
        offices_with_rows += 1
        max_page = min(max_pages, int(validation["max_page"] or 1))
        office_count = 0
        ...
        for row in rows:
            ...
            params = {
                **row,
                ...
                "raw_fields": json.dumps(row, ensure_ascii=False, default=str),
            }
            cur.execute(
                """
                INSERT INTO courses(
...
def run(
    *,
    ...
    if save and mark_stale and saved:
