"""Fail-closed collector for Dongducheon City's branded GSEEK catalogue.

The municipal landing page declares the complete offline-course total.  Its
JSON endpoint accepts one-based, end-exclusive ranges and returns nine rows
per request.  A snapshot is published only after the declared catalogue has
been read in full, the exact post-total range is empty, the first and last
range signatures remain stable, and every current/future detail page passes
the public course and application-control contracts.
...
DONGDUCHEON_PROVIDER = "MUNI_DDC_GSEEK_KR_97F9673C"
DONGDUCHEON_CANDIDATE_ID = "MUNI_IR_56EC8BC6C16E"
DONGDUCHEON_URL = "https://ddc.gseek.kr/user/course/offline/list"
DONGDUCHEON_HOST = "ddc.gseek.kr"
DONGDUCHEON_LIST_PATH = "/user/course/offline/list"
DONGDUCHEON_DETAIL_PATH = "/user/course/offline/view"
DONGDUCHEON_API_URL = "https://ddc.gseek.kr/user/course/offline/list/search"
DONGDUCHEON_PARENT_PROVIDER = "GYEONGGI_GSEEK"
DONGDUCHEON_PARENT_URL = "https://www.gseek.kr/user/course/offline/list"
DONGDUCHEON_PARENT_API_URL = "https://www.gseek.kr/user/course/offline/list/search"
DONGDUCHEON_REGION_CODE = "4125000000"
DONGDUCHEON_CO_SPONSOR_ID = "G000016"
DONGDUCHEON_BRANCH = "미래교육진흥원"
DONGDUCHEON_SOURCE_REGIONS = frozenset({"송내동", "동두천시"})
DONGDUCHEON_MUNICIPALITY_CODE = "4125000000"
DONGDUCHEON_MUNICIPALITY_NAME = "경기도 동두천시"
DONGDUCHEON_PAGE_SIZE = 9
DONGDUCHEON_DEFAULT_MAX_PAGES = 30
DONGDUCHEON_DEFAULT_DETAIL_LIMIT = 250
DONGDUCHEON_DEFAULT_MAX_REQUESTS = 360
DONGDUCHEON_SESSION_REQUEST_LIMIT = 90
DONGDUCHEON_PARSER = "dongducheon_gseek_post_census+exact_sentinel+stable_edges+current_detail"
...
class DongducheonContractError(ValueError):
...
def _positive_int(value: Any, label: str) -> int:
    try:
        ...
        raise DongducheonContractError(f"{label} must be a positive integer") from exc
    if parsed < 1:
        raise DongducheonContractError(f"{label} must be a positive integer")
    return parsed


def is_dongducheon_education_target(target: Any) -> bool:
    """Accept only the exact provider-owned HTTPS catalogue landing page."""

    parsed = urlparse(_target_url(target))
    return (
        _target_provider(target) == DONGDUCHEON_PROVIDER
        and parsed.scheme.lower() == "https"
        and (parsed.hostname or "").rstrip(".").lower() == DONGDUCHEON_HOST
        and parsed.port is None
        and parsed.path == DONGDUCHEON_LIST_PATH
...
is_target = is_dongducheon_education_target


def dongducheon_detail_url(subject_id: Any, cycle_id: Any) -> str:
    subject = _clean(subject_id)
    cycle = _clean(cycle_id)
    if not _IDENTITY_RE.fullmatch(subject) or not _IDENTITY_RE.fullmatch(cycle):
        return ""
    return f"https://{DONGDUCHEON_HOST}{DONGDUCHEON_DETAIL_PATH}?" + urlencode(
        {"s_sbjct_sn": subject, "s_sbjct_cycl_sn": cycle}
    )


def dongducheon_api_range(page: Any) -> tuple[int, int]:
    raw = _clean(page)
    if not _IDENTITY_RE.fullmatch(raw):
        return (0, 0)
    start = (int(raw) - 1) * DONGDUCHEON_PAGE_SIZE + 1
    return start, start + DONGDUCHEON_PAGE_SIZE


def dongducheon_sentinel_range(total: Any) -> tuple[int, int]:
...
def _validate_response(response: Any, expected_url: str) -> None:
    ...
    if status != 200:
        raise DongducheonContractError(f"unexpected HTTP status {status}")
    if getattr(response, "history", None):
        raise DongducheonContractError("HTTP redirects are not accepted")
    final_url = _clean(getattr(response, "url", ""))
    if final_url and final_url != expected_url:
        raise DongducheonContractError("response URL escaped the canonical route")
...
def _response_soup(response: Any, expected_url: str) -> BeautifulSoup:
    ...
    if not content:
        raise DongducheonContractError("empty HTML response")
...
def _response_json(response: Any, expected_url: str) -> list[Any]:
    ...
    if not isinstance(payload, list):
        raise DongducheonContractError("GSEEK API response is not a JSON list")
    return payload
...
class _Runner:
    def __init__(
        ...
        *,
        ...
        self._session_requests = DONGDUCHEON_SESSION_REQUEST_LIMIT
    ...
    def _ensure_session(self) -> None:
        if self._session is None or self._session_requests >= DONGDUCHEON_SESSION_REQUEST_LIMIT:
            self._new_session()
    ...
    def _attempt(self, operation: Callable[[Any], Any]) -> Any:
        ...
        for attempt in range(2):
            if self.physical_requests >= self._max_requests:
                raise DongducheonContractError(f"max_requests cap {self._max_requests} exhausted")
...
def _landing_total(soup: BeautifulSoup) -> Optional[int]:
    ...
    if (
        len(values) != 1
        or region is None
        or _clean(region.get("value")) != DONGDUCHEON_REGION_CODE
        or sponsor is None
        or _clean(sponsor.get("value")) != DONGDUCHEON_CO_SPONSOR_ID
        or DONGDUCHEON_LIST_PATH + "/search" not in markup
    ):
        return None
    return values.pop()
...
def _branch_code(branch: Any) -> str:
    ...
    return f"DONGDUCHEON_GSEEK_BRANCH_{digest}"
...
def _api_row(item: Mapping[str, Any]) -> tuple[dict[str, Any], list[str], bool]:
    ...
    test_or_notice = bool(_GENERIC_NON_COURSE_RE.fullmatch(title))
    ...
    if source_branch != DONGDUCHEON_BRANCH:
        errors.append(f"course {identity}: unaudited education institution")
    if source_region not in DONGDUCHEON_SOURCE_REGIONS:
        errors.append(f"course {identity}: non-Dongducheon source region")
    if _clean(item.get("d_co_sprvsn_id")) != DONGDUCHEON_CO_SPONSOR_ID:
        errors.append(f"course {identity}: non-Dongducheon site supervision")
    ...
    raw_url = dongducheon_detail_url(subject, cycle)
    period = f"{start.isoformat()} ~ {end.isoformat()}" if start and end else ""
    raw_fields: dict[str, Any] = {
        "parser": DONGDUCHEON_PARSER,
        "subject_id": subject,
        "cycle_id": cycle,
    ...
    row: dict[str, Any] = {
        "provider": DONGDUCHEON_PROVIDER,
        "provider_course_id": f"{DONGDUCHEON_PROVIDER}:course:{identity}",
        ...
        "program_type": "강좌",
        "region": DONGDUCHEON_MUNICIPALITY_NAME,
        "municipality_code": DONGDUCHEON_MUNICIPALITY_CODE,
        "municipality_full_name": DONGDUCHEON_MUNICIPALITY_NAME,
...
def _payload_signature(payload: list[Any]) -> str:
    ...
    for item in payload:
        if not isinstance(item, Mapping):
            raise DongducheonContractError("range signature encountered a non-object row")
...
def collect_dongducheon_education_courses(
    target: Any,
    timeout: int = 20,
    max_pages: int = DONGDUCHEON_DEFAULT_MAX_PAGES,
    detail_limit: int = DONGDUCHEON_DEFAULT_DETAIL_LIMIT,
    *,
    max_requests: int = DONGDUCHEON_DEFAULT_MAX_REQUESTS,
    ...
    """Return one atomic current/future Dongducheon education snapshot.
    ...
    meta = _base_meta()
    if not is_dongducheon_education_target(target):
        meta["configured_collection_error"] = "target does not match the canonical Dongducheon GSEEK route"
        return [], DONGDUCHEON_PARSER, meta
    ...
    try:
        try:
            landing_soup = runner.get_soup(DONGDUCHEON_URL)
        except Exception as exc:
        ...
        if landing_soup is not None:
            ...
            if declared is None or declared < 1:
                errors.append("landing page: missing unambiguous Dongducheon catalogue contract")
            else:
                source_total = declared
                data_pages = math.ceil(source_total / DONGDUCHEON_PAGE_SIZE)
        ...
        if not errors:
            for page in range(1, data_pages + 1):
                start, end = dongducheon_api_range(page)
                try:
                    ...
                    break
            if not errors:
                start, end = dongducheon_sentinel_range(source_total)
                try:
        ...
        if not errors:
            for page in range(1, data_pages + 1):
                ...
                expected = min(
                    DONGDUCHEON_PAGE_SIZE,
                    source_total - (page - 1) * DONGDUCHEON_PAGE_SIZE,
                )
        ...
        meta.update(
            {
                ...
                "sentinel_end": (sentinel_start + DONGDUCHEON_PAGE_SIZE if sentinel_start else 0),
                ...
                "max_requests": max_requests,
                "session_request_limit": DONGDUCHEON_SESSION_REQUEST_LIMIT,
                ...
                "no_current_reason": (
                    "all complete Dongducheon GSEEK catalogue courses have ended"
                    if snapshot_complete and not current_rows
                    else ""
                ),
                "configured_collection_error": "; ".join(errors),
                "candidate_id": DONGDUCHEON_CANDIDATE_ID,
                "ownership_region_code": DONGDUCHEON_REGION_CODE,
                "ownership_co_sponsor_id": DONGDUCHEON_CO_SPONSOR_ID,
                "ownership_branch": DONGDUCHEON_BRANCH,
                "parent_aggregate_provider": DONGDUCHEON_PARENT_PROVIDER,
                "parent_aggregate_exclusion_required": True,
                "parent_aggregate_exclusion_field": "d_co_sprvsn_id",
                "parent_aggregate_exclusion_value": DONGDUCHEON_CO_SPONSOR_ID,
        ...
        if errors:
            return [], DONGDUCHEON_PARSER, meta
        return result, DONGDUCHEON_PARSER, meta
    finally:
        runner.close()


collect = collect_dongducheon_education_courses


__all__ = [
    "DONGDUCHEON_API_URL",
    "DONGDUCHEON_BRANCH",
    "DONGDUCHEON_CANDIDATE_ID",
    "DONGDUCHEON_CO_SPONSOR_ID",
    "DONGDUCHEON_DEFAULT_DETAIL_LIMIT",
    "DONGDUCHEON_DEFAULT_MAX_PAGES",
    "DONGDUCHEON_DEFAULT_MAX_REQUESTS",
    "DONGDUCHEON_MUNICIPALITY_CODE",
    "DONGDUCHEON_MUNICIPALITY_NAME",
    "DONGDUCHEON_PAGE_SIZE",
    "DONGDUCHEON_PARENT_API_URL",
    "DONGDUCHEON_PARENT_PROVIDER",
    "DONGDUCHEON_PARENT_URL",
    "DONGDUCHEON_PARSER",
    "DONGDUCHEON_PROVIDER",
    "DONGDUCHEON_REGION_CODE",
    "DONGDUCHEON_SESSION_REQUEST_LIMIT",
    "DONGDUCHEON_SOURCE_REGIONS",
    "DONGDUCHEON_URL",
    "DongducheonContractError",
    "collect",
    "collect_dongducheon_education_courses",
    "dongducheon_api_range",
    "dongducheon_detail_url",
    "dongducheon_sentinel_range",
    "is_dongducheon_education_target",
    "is_target",
]
