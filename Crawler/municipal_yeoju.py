"""Fail-closed collector for Yeoju City's branded GSEEK catalogue.
...
YEOJU_PROVIDER = "MUNI_YEOJU_GSEEK_KR_1034027F"
YEOJU_CANDIDATE_ID = "MUNI_IR_734952B506D6"
YEOJU_URL = "https://yeoju.gseek.kr/user/course/offline/list"
YEOJU_HOST = "yeoju.gseek.kr"
YEOJU_LIST_PATH = "/user/course/offline/list"
YEOJU_DETAIL_PATH = "/user/course/offline/view"
YEOJU_API_URL = "https://yeoju.gseek.kr/user/course/offline/list/search"
YEOJU_PARENT_PROVIDER = "GYEONGGI_GSEEK"
YEOJU_PARENT_URL = "https://www.gseek.kr/user/course/offline/list"
YEOJU_PARENT_API_URL = "https://www.gseek.kr/user/course/offline/list/search"
YEOJU_REGION_CODE = "4167000000"
YEOJU_CO_SPONSOR_ID = "G000005"
YEOJU_BRANCH = "여주시평생학습관"
YEOJU_SOURCE_REGIONS = frozenset({"오학동", "중앙동", "북내면"})
YEOJU_MUNICIPALITY_CODE = "4167000000"
YEOJU_MUNICIPALITY_NAME = "경기도 여주시"
YEOJU_PAGE_SIZE = 9
YEOJU_DEFAULT_MAX_PAGES = 40
YEOJU_DEFAULT_DETAIL_LIMIT = 150
YEOJU_DEFAULT_MAX_REQUESTS = 180
YEOJU_SESSION_REQUEST_LIMIT = 90
YEOJU_PARSER = "yeoju_gseek_region_post_census+exact_sentinel+stable_edges+current_detail"
...
class YeojuContractError(ValueError):
...
def _positive_int(value: Any, label: str) -> int:
    try:
        ...
        raise YeojuContractError(f"{label} must be a positive integer") from exc
    if parsed < 1:
        raise YeojuContractError(f"{label} must be a positive integer")
    return parsed


def is_yeoju_education_target(target: Any) -> bool:
    """Accept only the exact provider-owned HTTPS catalogue landing page."""

    parsed = urlparse(_target_url(target))
    return (
        _target_provider(target) == YEOJU_PROVIDER
        and parsed.scheme.lower() == "https"
        and (parsed.hostname or "").rstrip(".").lower() == YEOJU_HOST
        and parsed.port is None
        and parsed.path == YEOJU_LIST_PATH
...
is_target = is_yeoju_education_target


def yeoju_detail_url(subject_id: Any, cycle_id: Any) -> str:
    subject = _clean(subject_id)
    cycle = _clean(cycle_id)
    if not _IDENTITY_RE.fullmatch(subject) or not _IDENTITY_RE.fullmatch(cycle):
        return ""
    return f"https://{YEOJU_HOST}{YEOJU_DETAIL_PATH}?" + urlencode(
        {"s_sbjct_sn": subject, "s_sbjct_cycl_sn": cycle}
    )


def yeoju_api_range(page: Any) -> tuple[int, int]:
    raw = _clean(page)
    if not _IDENTITY_RE.fullmatch(raw):
        return (0, 0)
    start = (int(raw) - 1) * YEOJU_PAGE_SIZE + 1
    return start, start + YEOJU_PAGE_SIZE


def yeoju_sentinel_range(total: Any) -> tuple[int, int]:
...
def _validate_response(response: Any, expected_url: str) -> None:
    ...
    if status != 200:
        raise YeojuContractError(f"unexpected HTTP status {status}")
    if getattr(response, "history", None):
        raise YeojuContractError("HTTP redirects are not accepted")
    final_url = _clean(getattr(response, "url", ""))
    if final_url and final_url != expected_url:
        raise YeojuContractError("response URL escaped the canonical route")
...
def _response_soup(response: Any, expected_url: str) -> BeautifulSoup:
    ...
    if not content:
        raise YeojuContractError("empty HTML response")
...
def _response_json(response: Any, expected_url: str) -> list[Any]:
    ...
    if not isinstance(payload, list):
        raise YeojuContractError("GSEEK API response is not a JSON list")
    return payload
...
class _Runner:
    def __init__(
        ...
        *,
        ...
        self._session_requests = YEOJU_SESSION_REQUEST_LIMIT
    ...
    def _ensure_session(self) -> None:
        if self._session is None or self._session_requests >= YEOJU_SESSION_REQUEST_LIMIT:
            self._new_session()
    ...
    def _attempt(self, operation: Callable[[Any], Any]) -> Any:
        ...
        for attempt in range(2):
            if self.physical_requests >= self._max_requests:
                raise YeojuContractError(f"max_requests cap {self._max_requests} exhausted")
...
def _landing_total(soup: BeautifulSoup) -> Optional[int]:
    ...
    if (
        len(values) != 1
        or region is None
        or _clean(region.get("value")) != YEOJU_REGION_CODE
        or sponsor is None
        or _clean(sponsor.get("value")) != YEOJU_CO_SPONSOR_ID
        or YEOJU_LIST_PATH + "/search" not in markup
    ):
        return None
    return values.pop()
...
def _branch_code(branch: Any) -> str:
    ...
    return f"YEOJU_GSEEK_BRANCH_{digest}"
...
def _api_row(item: Mapping[str, Any]) -> tuple[dict[str, Any], list[str], bool]:
    ...
    if source_branch != YEOJU_BRANCH:
        errors.append(f"course {identity}: unaudited education institution")
    if source_region not in YEOJU_SOURCE_REGIONS:
        errors.append(f"course {identity}: non-Yeoju source region")
    if _clean(item.get("d_co_sprvsn_id")) != YEOJU_CO_SPONSOR_ID:
        errors.append(f"course {identity}: non-Yeoju site supervision")
    ...
    raw_url = yeoju_detail_url(subject, cycle)
    ...
    row: dict[str, Any] = {
        "provider": YEOJU_PROVIDER,
        "provider_course_id": f"{YEOJU_PROVIDER}:course:{identity}",
        ...
        "program_type": "강좌",
        "region": YEOJU_MUNICIPALITY_NAME,
        "municipality_code": YEOJU_MUNICIPALITY_CODE,
        "municipality_full_name": YEOJU_MUNICIPALITY_NAME,
...
def _payload_signature(payload: list[Any]) -> str:
    ...
    for item in payload:
        if not isinstance(item, Mapping):
            raise YeojuContractError("range signature encountered a non-object row")
...
def collect_yeoju_education_courses(
    target: Any,
    timeout: int = 20,
    max_pages: int = YEOJU_DEFAULT_MAX_PAGES,
    detail_limit: int = YEOJU_DEFAULT_DETAIL_LIMIT,
    *,
    max_requests: int = YEOJU_DEFAULT_MAX_REQUESTS,
    ...
    """Return one atomic current/future Yeoju education snapshot.
    ...
    meta = _base_meta()
    if not is_yeoju_education_target(target):
        meta["configured_collection_error"] = "target does not match the canonical Yeoju GSEEK route"
        return [], YEOJU_PARSER, meta
    ...
    try:
        try:
            landing_soup = runner.get_soup(YEOJU_URL)
        except Exception as exc:
        ...
        if landing_soup is not None:
            ...
            if declared is None or declared < 1:
                errors.append("landing page: missing unambiguous Yeoju catalogue contract")
            else:
                source_total = declared
                data_pages = math.ceil(source_total / YEOJU_PAGE_SIZE)
        ...
        if not errors:
            for page in range(1, data_pages + 1):
                start, end = yeoju_api_range(page)
                try:
                    ...
                    break
            if not errors:
                start, end = yeoju_sentinel_range(source_total)
                try:
        ...
        if not errors:
            for page in range(1, data_pages + 1):
                ...
                expected = min(
                    YEOJU_PAGE_SIZE,
                    source_total - (page - 1) * YEOJU_PAGE_SIZE,
                )
        ...
        meta.update(
            {
                ...
                "sentinel_end": (sentinel_start + YEOJU_PAGE_SIZE if sentinel_start else 0),
                ...
                "max_requests": max_requests,
                "session_request_limit": YEOJU_SESSION_REQUEST_LIMIT,
                ...
                "no_current_reason": (
                    "all complete Yeoju GSEEK region-filtered catalogue courses have ended"
                    if snapshot_complete and not current_rows
                    else ""
                ),
                "configured_collection_error": "; ".join(errors),
                "candidate_id": YEOJU_CANDIDATE_ID,
                "ownership_region_code": YEOJU_REGION_CODE,
                "ownership_co_sponsor_id": YEOJU_CO_SPONSOR_ID,
                "ownership_branch": YEOJU_BRANCH,
                "parent_aggregate_provider": YEOJU_PARENT_PROVIDER,
                "parent_aggregate_exclusion_required": True,
                "parent_aggregate_exclusion_field": "d_co_sprvsn_id",
                "parent_aggregate_exclusion_value": YEOJU_CO_SPONSOR_ID,
        ...
        if errors:
            return [], YEOJU_PARSER, meta
        return result, YEOJU_PARSER, meta
    finally:
        runner.close()


collect = collect_yeoju_education_courses
...
__all__ = [
    ...
    "YEOJU_CANDIDATE_ID",
    "YEOJU_CO_SPONSOR_ID",
    "YEOJU_DEFAULT_DETAIL_LIMIT",
    "YEOJU_DEFAULT_MAX_PAGES",
    "YEOJU_DEFAULT_MAX_REQUESTS",
    "YEOJU_MUNICIPALITY_CODE",
    "YEOJU_MUNICIPALITY_NAME",
    "YEOJU_PAGE_SIZE",
    "YEOJU_PARENT_API_URL",
    "YEOJU_PARENT_PROVIDER",
    "YEOJU_PARENT_URL",
    "YEOJU_PARSER",
    "YEOJU_PROVIDER",
    "YEOJU_REGION_CODE",
    "YEOJU_SESSION_REQUEST_LIMIT",
    "YEOJU_SOURCE_REGIONS",
    "YEOJU_URL",
    "YeojuContractError",
    "collect",
    ...
    "yeoju_api_range",
    "yeoju_detail_url",
    "yeoju_sentinel_range",
    "is_yeoju_education_target",
    "is_target",
]
