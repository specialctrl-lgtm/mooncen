"""Fail-closed collector for Sangju City's integrated education ledger.

The retained owner is ``MUNI_WWW_SANGJU_GO_KR_A813366C``.  Its configured
pretty URL and ``/reserve/page/...`` are two views of the same integrated
``교육/강좌`` ledger; the former lifelong-learning provider and the separately
generated reservation provider are therefore aliases, not additional course
owners.  A discovered historical detail is also in this owner's ``cyclNo``
identity namespace and must never be promoted to a standalone provider.
...
identity-validated but ``apply.tc`` is never requested.  Instructor names,
contacts, free-text panels, images, and attachments are deliberately outside
...
SANGJU_PROVIDER = "MUNI_WWW_SANGJU_GO_KR_A813366C"
SANGJU_CANONICAL_CANDIDATE_ID = "MUNI_IR_482FB9F1BE5F"
SANGJU_DISCOVERY_DETAIL_CANDIDATE_ID = "MUNI_IR_1562731F2A97"
SANGJU_DISCOVERY_DETAIL_PROVIDER = "MUNI_WWW_SANGJU_GO_KR_6B30D50F"
SANGJU_DUPLICATE_RESERVATION_PROVIDER = "MUNI_WWW_SANGJU_GO_KR_AEA6F278"
SANGJU_LEGACY_LIFELONG_PROVIDER = "MUNI_WWW_SANGJU_GO_KR_B6294EFA"

SANGJU_MUNICIPALITY_CODE = "4725000000"
SANGJU_MUNICIPALITY_NAME = "경상북도 상주시"
SANGJU_HOST = "www.sangju.go.kr"
SANGJU_CANONICAL_URL = (
    f"https://{SANGJU_HOST}/page/15375/11881.tc?pageIndex=1"
)
SANGJU_CANONICAL_URL_SHA1 = (
    "a813366c2b8c393caf339e911e102728eacf3517"
)
SANGJU_CANONICAL_URL_SHA256 = (
    "01f38b8676556633a3d1bc877069cfd6a8a98a342db156df2932ae89b8765c9a"
)
SANGJU_DUPLICATE_RESERVATION_URL = (
    f"https://{SANGJU_HOST}/reserve/page/15375/11881.tc"
)
SANGJU_LEGACY_LIFELONG_URL = (
    f"https://{SANGJU_HOST}/le/page/15675/11632.tc"
)
SANGJU_DISCOVERY_DETAIL_URL = (
    f"https://{SANGJU_HOST}/reserve/reservation/detail.tc?"
    "mn=15375&searchTrgtClsfCd=RMS004001&searchFcltNo=&cyclNo=100456"
)
SANGJU_LIST_PATH = "/reserve/reservation/list.tc"
SANGJU_DETAIL_PATH = "/reserve/reservation/detail.tc"
SANGJU_APPLICATION_PATH = "/reserve/reservation/apply.tc"
SANGJU_PAGE_NO = "11881"
SANGJU_MENU_NO = "15375"
SANGJU_CLASS_CODE = "RMS004001"
SANGJU_PAGE_SIZE = 8
SANGJU_RECOMMENDED_MAX_PAGES = 100
SANGJU_RECOMMENDED_DETAIL_LIMIT = 100
SANGJU_MAX_WORKERS = 4
SANGJU_FETCH_ATTEMPTS = 2
SANGJU_MAX_HTML_BYTES = 3_000_000
SANGJU_PARSER = (
    "sangju_integrated_complete_current_education+exact_post_last_empty+"
    "twelve_facility_partition+all_current_public_details+stable_first_last_"
    "sentinel_recheck+cycl_rcpt_identity_controls_no_apply_fetch+pii_allowlist"
)
SANGJU_OWNERSHIP_SCOPE = (
    "sangju_city_integrated_reservation_complete_education_course_ledger"
)


class SangjuContractError(ValueError):
    """Raised when the official source violates its audited contract."""


@dataclass(frozen=True)
class SangjuFacility:
    code: str
    name: str
...
SANGJU_FACILITIES: tuple[SangjuFacility, ...] = (
    SangjuFacility("100001", "아이여성행복과"),
    SangjuFacility("100006", "경상북도교육청 상주도서관"),
    SangjuFacility("127", "상주문화회관"),
    SangjuFacility("129", "상주시립도서관(생활문화센터)"),
    SangjuFacility("130", "상주시보건소"),
    SangjuFacility("132", "상주시농촌신활력플러스사업추진단"),
    SangjuFacility("131", "상주시 평생학습관"),
    SangjuFacility("135", "상주시 청소년 해양교육원"),
    SangjuFacility("106", "태평성대 경상감영공원"),
    SangjuFacility("119", "상주시농업기술센터"),
    SangjuFacility("84", "상주시 정보화 교육장"),
    SangjuFacility("113", "상주박물관"),
)
SANGJU_FACILITY_BY_CODE = {item.code: item for item in SANGJU_FACILITIES}
SANGJU_FACILITY_BY_NAME = {item.name: item for item in SANGJU_FACILITIES}

SANGJU_CANDIDATE_AUDIT: Mapping[str, Mapping[str, str]] = {
    SANGJU_CANONICAL_CANDIDATE_ID: {
        "provider": SANGJU_PROVIDER,
        "url": SANGJU_CANONICAL_URL,
        "decision": "retain_incumbent_provider_and_collect_complete_integrated_ledger",
    },
    SANGJU_DISCOVERY_DETAIL_CANDIDATE_ID: {
        "provider": SANGJU_PROVIDER,
        "discovered_provider": SANGJU_DISCOVERY_DETAIL_PROVIDER,
        "url": SANGJU_DISCOVERY_DETAIL_URL,
        "decision": "same_owner_historical_detail_alias; never_create_separate_provider",
    },
}
SANGJU_PROVIDER_ALIAS_AUDIT: Mapping[str, Mapping[str, str]] = {
    SANGJU_DUPLICATE_RESERVATION_PROVIDER: {
        "url": SANGJU_DUPLICATE_RESERVATION_URL,
        "state": "superseded",
        "canonical_provider": SANGJU_PROVIDER,
        "decision": "same_exact_integrated_education_ledger",
    },
    SANGJU_LEGACY_LIFELONG_PROVIDER: {
        "url": SANGJU_LEGACY_LIFELONG_URL,
        "state": "superseded",
        "canonical_provider": SANGJU_PROVIDER,
        "decision": "legacy_navigation_shell_links_to_facility_131; no_own_course_ids",
    },
}
SANGJU_OWNER_BOUNDARIES: tuple[Mapping[str, str], ...] = (
    {
        "url": SANGJU_DUPLICATE_RESERVATION_URL,
        "decision": "same_owner_duplicate_pretty_route",
    },
    {
        "url": SANGJU_LEGACY_LIFELONG_URL,
        "decision": "same_owner_legacy_navigation_alias_not_a_ledger",
    },
    {
        "url": SANGJU_DISCOVERY_DETAIL_URL,
        "decision": "same_cyclNo_namespace_historical_detail_not_a_provider",
    },
)

SANGJU_LIVE_AUDIT_BASELINE: Mapping[str, Any] = {
    "cutoff": "2026-07-23",
    "source_total": 237,
    "current_total": 43,
    "standing_source_total": 11,
    "data_pages": 30,
    "page_sizes": [*([8] * 29), 5],
    "post_last_page": 31,
    "facility_counts": {
    ...
    "current_facility_counts": {"131": 24, "129": 11, "84": 5, "100006": 3},
    "status_counts": {"SCHEDULED": 24, "OPEN": 8, "CLOSED": 11},
    "current_ids": [
        "100763", "100762", "100761", "100760", "100759", "100758",
        "100757", "100756", "100755", "100754", "100753", "100752",
        "100751", "100750", "100749", "100748", "100747", "100746",
        "100745", "100744", "100743", "100742", "100741", "100740",
        "100739", "100738", "100737", "100734", "100733", "100708",
        "100694", "100693", "100689", "100673", "100672", "100671",
        "100670", "100669", "100668", "100667", "100649", "100648",
        "100587",
    ],
    "list_requests": 69,
    "detail_requests": 43,
    "source_requests": 112,
    "two_snapshot_requests": 224,
}

SANGJU_FIELDS_NEVER_PERSISTED = (
    "강사",
    "담당자·연락처·전화번호·이메일",
...
_AUDITED_REVERSED_HISTORICAL_PERIOD_IDS = frozenset({"414"})
_SAFE_RAW_FIELDS = frozenset(
    {
        "identity", "source_page", "source_position", "source_ordinal",
        "source_category", "source_facility_code", "source_facility_name",
        "source_address", "source_status", "source_education_period",
        "source_operating_schedule", "source_reception_heading",
        "source_apply_period", "source_capacity_current", "source_capacity_total",
        "source_waitlist_current", "source_waitlist_total", "source_rcpt_no",
        "list_identity_verified", "facility_partition_verified",
        "detail_identity_verified", "detail_structured_fields_verified",
        "detail_reception_verified", "application_control_present",
        "application_control_verified", "application_endpoint_fetched",
        "application_form_submitted", "attachment_endpoint_fetched",
        "free_text_persisted", "discarded_fields", "service_family",
    }
)
_FORBIDDEN_ROW_KEYS = frozenset(
    {
        "instructor", "teacher", "manager", "contact", "phone", "email",
        "attachments", "attachment_url", "image_url", "body", "content_html",
        "guide", "notice", "applicant_name", "resident_number",
    }
)
...
def _strict_target_url(url: str) -> bool:
    ...
    return (
        parsed.scheme == "https"
        and parsed.netloc == SANGJU_HOST
        and parsed.path == "/page/15375/11881.tc"
        and parsed.params == ""
        and parsed.fragment == ""
...
def is_sangju_education_target(target: Any) -> bool:
    return (
        _clean(_target_value(target, "provider")) == SANGJU_PROVIDER
        and _strict_target_url(_clean(_target_value(target, "url")))
    )


is_target = is_sangju_education_target
...
def _raw_session() -> requests.Session:
    ...
    session.headers.update(
        {
            "User-Agent": "Mozilla/5.0 (compatible; MoonCen-Sangju-Audit/1.0)",
...
def _list_url(page: int, facility_code: str = "") -> str:
    query = urlencode(
        (
            ("pageNo", SANGJU_PAGE_NO),
            ("mn", SANGJU_MENU_NO),
            ("searchTrgtClsfCd", SANGJU_CLASS_CODE),
            ("searchFcltNo", facility_code),
            ("pageIndex", str(page)),
        )
    )
    return f"https://{SANGJU_HOST}{SANGJU_LIST_PATH}?{query}"
...
def _detail_url(identity: str) -> str:
    ...
    return f"https://{SANGJU_HOST}{SANGJU_DETAIL_PATH}?{query}"
...
def _application_url(identity: str, rcpt_no: str) -> str:
    ...
    return f"https://{SANGJU_HOST}{SANGJU_APPLICATION_PATH}?{query}"


def _validate_fetch_url(url: str) -> str:
    parsed = urlparse(url)
    if parsed.scheme != "https" or parsed.netloc != SANGJU_HOST or parsed.fragment:
        raise SangjuContractError("request escaped exact Sangju HTTPS host")
    pairs = parse_qsl(parsed.query, keep_blank_values=True)
    if parsed.path == SANGJU_LIST_PATH:
        expected_keys = [
            "pageNo", "mn", "searchTrgtClsfCd", "searchFcltNo", "pageIndex"
        ]
        if [key for key, _ in pairs] != expected_keys:
            raise SangjuContractError("list query shape/order drift")
        values = dict(pairs)
        if (
            values["pageNo"] != SANGJU_PAGE_NO
            or values["mn"] != SANGJU_MENU_NO
            or values["searchTrgtClsfCd"] != SANGJU_CLASS_CODE
            or values["searchFcltNo"] not in {"", *SANGJU_FACILITY_BY_CODE}
            or not _IDENTITY.fullmatch(values["pageIndex"])
        ):
            raise SangjuContractError("list query value drift")
        return "list"
    if parsed.path == SANGJU_DETAIL_PATH:
        expected_keys = [
            "mn", "pageNo", "searchTrgtClsfCd", "searchFcltNo", "cyclNo"
        ]
        if [key for key, _ in pairs] != expected_keys:
            raise SangjuContractError("detail query shape/order drift")
        values = dict(pairs)
        if (
            values["mn"] != SANGJU_MENU_NO
            or values["pageNo"] != SANGJU_PAGE_NO
            or values["searchTrgtClsfCd"] != SANGJU_CLASS_CODE
            or values["searchFcltNo"] != ""
            or not _IDENTITY.fullmatch(values["cyclNo"])
        ):
            raise SangjuContractError("detail query value drift")
        return "detail"
    raise SangjuContractError("request escaped audited list/detail endpoints")
...
def _validate_owner_shell(soup: BeautifulSoup, kind: str) -> None:
    ...
    if "상주시" not in title or "통합예약 홈페이지" not in title:
        raise SangjuContractError(f"{kind} owner/title shell drift")
    page_text = _clean(soup.get_text(" ", strip=True))
    if "(37211) 경상북도 상주시 상산로 223(남성동 140-3)" not in page_text:
        raise SangjuContractError(f"{kind} official owner footer drift")
...
def _fetch_soup(
    ...
    for attempt in range(1, SANGJU_FETCH_ATTEMPTS + 1):
        try:
            ...
            if getattr(response, "history", []):
                raise SangjuContractError("redirect history is not allowed")
            actual_url = _clean(getattr(response, "url", ""))
            if not _same_response_url(actual_url, url):
                raise SangjuContractError("response URL drift")
            content = getattr(response, "content", b"")
            if not isinstance(content, (bytes, bytearray)):
                raise SangjuContractError("response body is not bytes")
            if not content or len(content) > SANGJU_MAX_HTML_BYTES:
                raise SangjuContractError("response body size outside audited bounds")
            try:
                html = bytes(content).decode("utf-8", errors="strict")
            except UnicodeDecodeError as exc:
                raise SangjuContractError("response is not strict UTF-8") from exc
            soup = BeautifulSoup(html, "html.parser")
            _validate_owner_shell(soup, kind)
            return soup, attempt, kind
        except SangjuContractError:
            raise
        except requests.RequestException as exc:
            last_error = exc
    raise SangjuContractError(f"request failed after retries: {_clean(last_error)}")
...
def _field_map(container: Any, *, direct_ul: bool = True) -> tuple[dict[str, str], tuple[str, ...]]:
    ...
    for candidate in candidates:
        if "tm_cir" in candidate.get("class", []):
            ...
            break
    if ul is None:
        raise SangjuContractError("structured tm_cir field list missing")
    result: dict[str, str] = {}
    order: list[str] = []
    for item in ul.find_all("li", recursive=False):
        label_node = item.find("span", recursive=False)
        if label_node is None:
            raise SangjuContractError("structured field label missing")
        label = _label_text(label_node)
        if label in result:
            raise SangjuContractError(f"duplicate structured field: {label}")
...
def _parse_iso_period(value: str, identity: str) -> tuple[date, Optional[date]]:
    ...
    if len(matches) != 2:
        raise SangjuContractError(f"course {identity}: operating date shape drift")
    ...
    if values[0] > values[1] and identity not in _AUDITED_REVERSED_HISTORICAL_PERIOD_IDS:
        raise SangjuContractError(f"course {identity}: reversed operating period")
    return values[0], values[1]


def _parse_korean_datetimes(
    value: str, identity: str
) -> tuple[datetime, Optional[datetime]]:
    matches = _KOREAN_DATETIME.findall(value)
    if len(matches) == 1 and "상시운영" in value:
        return datetime(*(int(part) for part in matches[0])), None
    if len(matches) != 2:
        raise SangjuContractError(f"course {identity}: reception datetime shape drift")
    values = [datetime(*(int(part) for part in match)) for match in matches]
    if values[0] > values[1]:
        raise SangjuContractError(f"course {identity}: reversed reception period")
    return values[0], values[1]
...
def _parse_count(value: str, identity: str, label: str) -> tuple[int, int]:
    ...
    if not match:
        raise SangjuContractError(f"course {identity}: {label} count shape drift")
    current, total = (int(part.replace(",", "")) for part in match.groups())
    if current < 0 or total < 0:
        raise SangjuContractError(f"course {identity}: negative {label} count")
...
def _normal_action(anchor: Any, namespace: str, identity: str) -> tuple[str, str]:
    ...
    if href != "javascript:;":
        raise SangjuContractError(f"course {identity}: reception control href drift")
    if text == "예약":
        ...
        if match is None or classes:
            raise SangjuContractError(f"course {identity}: active reservation control drift")
        return text, match.group(1)
    if text == "대기":
        if onclick != "alert('접수 대기 중입니다.');" or classes != {"bg_gray"}:
            raise SangjuContractError(f"course {identity}: waiting control drift")
        return text, ""
    if text == "종료":
        if onclick not in {
            "alert('접수가 종료되었습니다.')",
            ...
            raise SangjuContractError(f"course {identity}: ended control drift")
        return text, ""
    raise SangjuContractError(f"course {identity}: unknown reception control {text}")
...
def _parse_reception_block(block: Any, namespace: str, identity: str) -> dict[str, Any]:
    ...
    if order == ("[예약불가]",):
        if (
            heading_node is not None
            or anchors
            or fields["[예약불가]"] != "등록된 접수 정보가 없습니다."
        ):
            raise SangjuContractError(f"course {identity}: reservation-unavailable drift")
        return {
            "heading": "",
            "status": "예약불가",
            "rcpt_no": "",
    ...
    if order not in _RECEPTION_FIELD_SHAPES or heading_node is None or len(anchors) != 1:
        raise SangjuContractError(f"course {identity}: reception block field/control drift")
    heading = _clean(heading_node.get_text(" ", strip=True))
    if not heading:
        raise SangjuContractError(f"course {identity}: empty reception heading")
...
def _parse_receptions(container: Any, namespace: str, identity: str) -> list[dict[str, Any]]:
    ...
    if not blocks:
        raise SangjuContractError(f"course {identity}: empty reception container")
...
def _reception_signature(item: Mapping[str, Any]) -> tuple[Any, ...]:
    return (
        item["heading"], item["status"], item["rcpt_no"],
        item["apply_start"], item["apply_end"], item["capacity_current"],
        item["capacity_total"], item["waitlist_current"], item["waitlist_total"],
    )


def _aggregate_status(badge: str, receptions: list[dict[str, Any]], identity: str) -> str:
    statuses = {str(item["status"]) for item in receptions}
    if not statuses:
        if badge == "정보제공":
            return "정보제공"
        if badge:
            raise SangjuContractError(f"course {identity}: unknown top badge {badge}")
        return "정보없음"
    if statuses == {"예약불가"}:
        if badge:
            raise SangjuContractError(f"course {identity}: unavailable/badge disagreement")
        return "예약불가"
    if len(statuses) != 1 or statuses - {"예약", "대기", "종료"}:
        raise SangjuContractError(f"course {identity}: mixed reception statuses")
    status = next(iter(statuses))
    expected_badge = "온라인예약 접수중" if status == "예약" else ""
    if badge != expected_badge:
        raise SangjuContractError(f"course {identity}: list badge/control disagreement")
    return status
...
def _parse_card(section: Any, requested_page: int, position: int) -> dict[str, Any]:
    ...
    if right is None:
        raise SangjuContractError("course card right panel missing")
    heading = right.find("h1", recursive=False)
    if heading is None:
        raise SangjuContractError("course card heading missing")
    ...
    if ordinal_node is None or title_anchor is None:
        raise SangjuContractError("course card ordinal/title control missing")
    ordinal_text = _clean(ordinal_node.get_text(" ", strip=True))
    if not _ORDINAL.fullmatch(ordinal_text):
        raise SangjuContractError("course card ordinal drift")
    title = _clean(title_anchor.get_text(" ", strip=True))
    if not title:
        raise SangjuContractError("empty course title")
    actions = []
    ...
    if len(actions) != 2 or len(set(actions)) != 1:
        raise SangjuContractError("course list/detail identity controls drift")
    identity = actions[0]
    fields, order = _field_map(right)
    if order != _LIST_FIELD_LABELS or any(not fields[label] for label in _LIST_FIELD_LABELS):
        raise SangjuContractError(f"course {identity}: list field contract drift")
    if not fields["분류"].startswith("교육/강좌"):
        raise SangjuContractError(f"course {identity}: escaped education classification")
    if fields["시설명"] not in SANGJU_FACILITY_BY_NAME:
        raise SangjuContractError(f"course {identity}: unknown facility")
    ...
    raw_status = _aggregate_status(badge, receptions, identity)
    return {
        "identity": identity,
...
def _named_hidden(form: Any) -> dict[str, str]:
    ...
    for node in form.select('input[type="hidden"][name]'):
        ...
        if not name or name in result:
            raise SangjuContractError("hidden form field missing/duplicated")
...
def _validate_facility_registry(soup: BeautifulSoup, selected_code: str) -> None:
    ...
    expected = [("", "전체"), *((item.code, item.name) for item in SANGJU_FACILITIES)]
    actual: list[tuple[str, str]] = []
    active: list[str] = []
    for button in buttons:
        match = _FACILITY_ACTION.fullmatch(_clean(button.get("onclick")))
        if match is None or _clean(button.get("type")) != "button":
            raise SangjuContractError("facility navigation action drift")
        code = match.group(1)
...
def _parse_pager(soup: BeautifulSoup, requested_page: int) -> tuple[int, Optional[int]]:
    ...
    if href_pairs != [("pageIndex", str(advertised_last))]:
        raise SangjuContractError("pager last href drift")
    active_nodes = pager.select("a.active")
    if len(active_nodes) > 1:
        raise SangjuContractError("multiple active pager entries")
    current = None
    if active_nodes:
        value = _clean(active_nodes[0].get_text(" ", strip=True))
        if not _IDENTITY.fullmatch(value):
            raise SangjuContractError("active pager value drift")
        current = int(value)
    if current is not None and current != requested_page:
        raise SangjuContractError("active pager/request disagreement")
...
def _parse_list_page(
    ...
    *,
    ...
    if form is None or form.get("action") is not None or form.get("method") is not None:
        raise SangjuContractError("list form contract drift")
    expected_hidden = {
        "pageNo": SANGJU_PAGE_NO,
        "mn": SANGJU_MENU_NO,
    ...
    if _named_hidden(form) != expected_hidden:
        raise SangjuContractError("list hidden form binding drift")
    ...
    if ledger is None:
        raise SangjuContractError("reserveList ledger missing")
    sections = ledger.find_all("section", recursive=False)
    if not sections:
        raise SangjuContractError("reserveList sections missing")
    empty = False
    if len(sections) == 1 and sections[0].select_one(":scope > p.no_data") is not None:
        ...
        if _clean(empty_node.get_text(" ", strip=True)) != "자료가 없습니다.":
            raise SangjuContractError("empty sentinel text drift")
        if sections[0].find("h1") is not None:
            raise SangjuContractError("empty sentinel contains course data")
        rows: list[dict[str, Any]] = []
        empty = True
    else:
        if any(section.select_one(":scope > p.no_data") for section in sections):
            raise SangjuContractError("mixed data/empty page")
        rows = [
            _parse_card(section, requested_page, position)
            for position, section in enumerate(sections, 1)
        ]
    if empty and current_page is None and requested_page <= advertised_last:
        raise SangjuContractError("advertised data page lost active pager")
    if rows and current_page != requested_page:
        raise SangjuContractError("data page lacks matching active pager")
    return {
...
def _row_signature(row: Mapping[str, Any]) -> tuple[Any, ...]:
    return (
        row["identity"], row["title"], row["source_category"],
        row["facility_name"], row["address"], row["event_start"],
        row["event_end"], row["badge"], row["raw_status"],
...
def _page_signature(page: Mapping[str, Any]) -> tuple[Any, ...]:
    return (
        page["requested_page"], page["facility_code"], page["advertised_last"],
        page["current_page"], page["empty"],
...
def _collect_advertised_pages(
    ...
    *,
    ...
    expected_last = max(1, (total + SANGJU_PAGE_SIZE - 1) // SANGJU_PAGE_SIZE)
    if last != expected_last:
        raise SangjuContractError("ordinal total/pager last disagreement")
    for index, page in enumerate(pages, 1):
        if page["requested_page"] != index or page["advertised_last"] != last:
            raise SangjuContractError("pagination metadata drift")
        expected_size = 0 if total == 0 else min(
            SANGJU_PAGE_SIZE, total - SANGJU_PAGE_SIZE * (index - 1)
        )
        if len(page["rows"]) != expected_size or bool(page["empty"]) != (expected_size == 0):
            raise SangjuContractError("page-size/empty boundary drift")
        expected_ordinals = list(
            range(
                total - SANGJU_PAGE_SIZE * (index - 1),
                total - SANGJU_PAGE_SIZE * (index - 1) - expected_size,
                -1,
            )
        )
        if [row["ordinal"] for row in page["rows"]] != expected_ordinals:
            raise SangjuContractError("global course ordinal sequence drift")
    rows = [row for page in pages for row in page["rows"]]
    identities = [str(row["identity"]) for row in rows]
    if len(rows) != total or len(identities) != len(set(identities)):
        raise SangjuContractError("ledger total/identity cardinality drift")
    return rows, pages
...
def _parse_detail(soup: BeautifulSoup, expected_row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if form is None:
        raise SangjuContractError(f"course {identity}: detail form missing")
    expected_hidden = {
        "pageNo": SANGJU_PAGE_NO,
        "mn": SANGJU_MENU_NO,
        "pageIndex": "",
    ...
    if _named_hidden(form) != expected_hidden:
        raise SangjuContractError(f"course {identity}: detail identity binding drift")
    root = form.select_one(".img_jb > .right")
    if root is None:
        raise SangjuContractError(f"course {identity}: detail header missing")
    ...
    if title_node is None or state_node is None:
        raise SangjuContractError(f"course {identity}: detail title/state missing")
    ...
    if (
        ...
        raise SangjuContractError(f"course {identity}: detail field set drift")
    ...
    if (
        ...
        raise SangjuContractError(f"course {identity}: list/detail structured data drift")
    expected_state = _DETAIL_STATE_BY_SOURCE.get(str(expected_row["raw_status"]))
    if expected_state is None or state != expected_state:
        raise SangjuContractError(f"course {identity}: list/detail state drift")
    motion = form.select_one(".motion_wrap")
    if motion is None:
        if expected_row["receptions"]:
            raise SangjuContractError(f"course {identity}: detail reception area missing")
        receptions: list[dict[str, Any]] = []
    else:
        receptions = _parse_receptions(motion, "reserveDetail", identity)
    if tuple(map(_reception_signature, receptions)) != tuple(
        map(_reception_signature, expected_row["receptions"])
    ):
        raise SangjuContractError(f"course {identity}: list/detail reception drift")
    back_controls = [
        anchor
        for anchor in form.select(".bot_btn a")
        if _clean(anchor.get("onclick")) == "reserveDetail.list();"
    ]
    if len(back_controls) != 1:
        raise SangjuContractError(f"course {identity}: detail return control drift")
    panels = form.select(".tabpanel_wrap .bd_scroll")
    if len(panels) != 3:
        raise SangjuContractError(f"course {identity}: free-text panel boundary drift")
    return {
        "identity": identity,
        "state": state,
...
def _current_semantics(row: Mapping[str, Any]) -> None:
    ...
    if status not in _DETAIL_STATE_BY_SOURCE:
        raise SangjuContractError(f"course {identity}: unsupported current status {status}")
    if status == "정보제공":
        if receptions or row["badge"] != "정보제공":
            raise SangjuContractError(f"course {identity}: information-only control drift")
        return
    if len(receptions) != 1 or receptions[0]["status"] != status:
        raise SangjuContractError(f"course {identity}: current reception cardinality drift")
    if status == "예약" and not receptions[0]["rcpt_no"]:
        raise SangjuContractError(f"course {identity}: active rcptNo missing")
    if status != "예약" and receptions[0]["rcpt_no"]:
        raise SangjuContractError(f"course {identity}: inactive rcptNo present")
...
def _output_row(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    detail = row["detail"]
    facility = SANGJU_FACILITY_BY_NAME[str(row["facility_name"])]
    receptions = list(row["receptions"])
    ...
    apply_period = (
        f"{_iso_minute(apply_start)} ~ {_iso_minute(apply_end)}" if apply_start else ""
    )
    ...
    return {
        "provider": SANGJU_PROVIDER,
        "provider_course_id": f"{SANGJU_PROVIDER}:cycl:{identity}",
        ...
        "capacity_remaining": (
            ...
            if capacity_total is not None and capacity_current is not None else None
        ),
        ...
        "collection_type": SANGJU_PARSER,
        "municipality_code": SANGJU_MUNICIPALITY_CODE,
        "municipality_full_name": SANGJU_MUNICIPALITY_NAME,
        "raw_fields": {
            "identity": identity,
            ...
            "discarded_fields": list(SANGJU_FIELDS_NEVER_PERSISTED),
...
def _initial_meta() -> dict[str, Any]:
    return {
        "provider": SANGJU_PROVIDER,
        "provider_decision": (
            "retain incumbent provider and replace partial generic collection with "
            "the complete integrated education ledger"
        ),
        "canonical_url": SANGJU_CANONICAL_URL,
        "canonical_url_sha1": SANGJU_CANONICAL_URL_SHA1,
        "canonical_url_sha256": SANGJU_CANONICAL_URL_SHA256,
        "canonical_candidate_id": SANGJU_CANONICAL_CANDIDATE_ID,
        "discovery_detail_candidate_id": SANGJU_DISCOVERY_DETAIL_CANDIDATE_ID,
        "parser": SANGJU_PARSER,
        "ownership_scope": SANGJU_OWNERSHIP_SCOPE,
        "municipality_code": SANGJU_MUNICIPALITY_CODE,
        "municipality_full_name": SANGJU_MUNICIPALITY_NAME,
        "page_size": SANGJU_PAGE_SIZE,
        "recommended_max_pages": SANGJU_RECOMMENDED_MAX_PAGES,
        "recommended_detail_limit": SANGJU_RECOMMENDED_DETAIL_LIMIT,
        "source_requests": 0,
        "list_requests": 0,
        "detail_requests": 0,
        "request_attempts": 0,
...
def collect_sangju_education(
    target: Any,
    *,
    timeout: int = 30,
    max_pages: int = SANGJU_RECOMMENDED_MAX_PAGES,
    detail_limit: int = SANGJU_RECOMMENDED_DETAIL_LIMIT,
    ...
    """Collect one complete, stable, privacy-safe Sangju education snapshot."""

    meta = _initial_meta()
    if not is_sangju_education_target(target):
        meta["configured_collection_error"] = "target does not match exact retained Sangju owner"
        return [], SANGJU_PARSER, meta
    ...
    def fetch_page(page: int, facility_code: str) -> dict[str, Any]:
        soup, attempts, _ = _fetch_soup(
            main_session, _list_url(page, facility_code), timeout, current_fetcher
        )
        meta["source_requests"] += 1
        meta["list_requests"] += 1
        meta["request_attempts"] += attempts
        return _parse_list_page(
            soup, requested_page=page, facility_code=facility_code
        )
    ...
    try:
        listed, initial_pages = _collect_advertised_pages(
            fetch_page, facility_code="", max_pages=max_pages
        )
        ...
        if (
            ...
            raise SangjuContractError("post-last page is not the exact empty sentinel")
        source_by_id = {str(row["identity"]): row for row in listed}
        if len(source_by_id) != len(listed):
            raise SangjuContractError("duplicate source identity")
        ...
        overlap_count = 0
        for facility in SANGJU_FACILITIES:
            filtered, pages = _collect_advertised_pages(
                fetch_page, facility_code=facility.code, max_pages=max_pages
            )
            ...
            for filtered_row in filtered:
                ...
                if identity in memberships:
                    overlap_count += 1
                    raise SangjuContractError(f"facility partitions overlap at {identity}")
                source_row = source_by_id.get(identity)
                if source_row is None:
                    raise SangjuContractError(f"facility partition escapes source at {identity}")
                if _row_signature(filtered_row) != _row_signature(source_row):
                    raise SangjuContractError(f"facility partition data drift at {identity}")
                if filtered_row["facility_name"] != facility.name:
                    raise SangjuContractError(f"facility filter/name disagreement at {identity}")
                memberships[identity] = facility.code
        if set(memberships) != set(source_by_id):
            missing = sorted(set(source_by_id) - set(memberships))
            raise SangjuContractError(
                "facility partition union incomplete: " + ", ".join(missing[:5])
            )
        for row in listed:
        ...
        for row in listed:
            for reception in row["receptions"]:
                ...
                if rcpt_no in active_rcpt:
                    raise SangjuContractError("rcptNo reused across course identities")
        ...
        if current_rows and fetcher is None and len(current_rows) > 1:
            chunks = [current_rows[index::SANGJU_MAX_WORKERS] for index in range(SANGJU_MAX_WORKERS)]
            ...
            with ThreadPoolExecutor(max_workers=SANGJU_MAX_WORKERS) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    for identity, parsed in parsed_batch:
                        if identity in detail_results:
                            raise SangjuContractError("duplicate detail result identity")
        ...
        if set(detail_results) != {str(row["identity"]) for row in current_rows}:
            raise SangjuContractError("current detail result union incomplete")
        ...
        if (
            ...
            raise SangjuContractError("source boundaries changed during detail collection")

        rows = [_output_row(row) for row in current_rows]
        privacy_failures = [error for row in rows for error in _privacy_errors(row)]
        if privacy_failures:
            raise SangjuContractError("; ".join(sorted(set(privacy_failures))))
        ...
        if len(after_ids) != len(set(after_ids)) or set(after_ids) != before_ids:
            raise SangjuContractError("dedupe_rows changed complete identity cardinality")
        privacy_failures = [error for row in rows for error in _privacy_errors(row)]
        if privacy_failures:
            raise SangjuContractError("; ".join(sorted(set(privacy_failures))))
        ...
        meta.update(
            {
                ...
                "facility_partition_counts": {
                    facility.code: len(facility_rows[facility.code])
                    for facility in SANGJU_FACILITIES
                },
                ...
                "empty_facility_filter_count": sum(
                    not facility_rows[facility.code] for facility in SANGJU_FACILITIES
                ),
                ...
                "attachment_links_discarded": sum(
                    int(item["attachment_count"]) for item in detail_results.values()
                ),
                "images_discarded": sum(
                    int(item["image_count"]) for item in detail_results.values()
                ),
        ...
        return rows, SANGJU_PARSER, meta
...
collect = collect_sangju_education
...
__all__ = [
    ...
    "SANGJU_CANONICAL_URL_SHA1",
    "SANGJU_CANONICAL_URL_SHA256",
    "SANGJU_CANDIDATE_AUDIT",
    "SANGJU_DISCOVERY_DETAIL_CANDIDATE_ID",
    "SANGJU_DISCOVERY_DETAIL_PROVIDER",
    "SANGJU_DISCOVERY_DETAIL_URL",
    "SANGJU_DUPLICATE_RESERVATION_PROVIDER",
    "SANGJU_DUPLICATE_RESERVATION_URL",
    "SANGJU_FACILITIES",
    ...
    "SANGJU_LEGACY_LIFELONG_PROVIDER",
    "SANGJU_LEGACY_LIFELONG_URL",
    "SANGJU_LIVE_AUDIT_BASELINE",
    "SANGJU_MUNICIPALITY_CODE",
    "SANGJU_MUNICIPALITY_NAME",
    "SANGJU_OWNER_BOUNDARIES",
    "SANGJU_OWNERSHIP_SCOPE",
    "SANGJU_PARSER",
    "SANGJU_PROVIDER",
    "SANGJU_PROVIDER_ALIAS_AUDIT",
    "SANGJU_RECOMMENDED_DETAIL_LIMIT",
    "SANGJU_RECOMMENDED_MAX_PAGES",
    "SangjuContractError",
    "collect",
    "collect_sangju_education",
    "is_sangju_education_target",
    "is_target",
]
