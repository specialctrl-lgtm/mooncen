...
SPECIFIC_RECEPTION_LABEL_RE = re.compile(r"(기간|일시|시작|종료)")
RECEPTION_STATUS_SUFFIX_RE = re.compile(r"\s*(중|마감|완료|종료|대기)")
...
def extract_reception_period(text: Any, reference_date: Any = None) -> dict[str, Any]:
    ...
    def add_candidate(
        ...
        normalized_label = re.sub(r"\s+", "", label_match.group(0))
        if (
            normalized_label in {"접수", "신청", "모집", "등록"}
            and RECEPTION_STATUS_SUFFIX_RE.match(following)
        ):
            return
        destination = (
            specific_candidates
            if SPECIFIC_RECEPTION_LABEL_RE.search(normalized_label)
            else generic_candidates
        )
        destination.append(snippet)
    ...
    for label_match in RECEPTION_LABEL_RE.finditer(source):
        ...
        if DATE_TOKEN_RE.search(snippet):
            add_candidate(
                label_match,
                snippet,
                source[label_match.end():label_match.end() + 8],
            )

    for chunk in chunks:
        if EXCLUDED_LABEL_RE.search(chunk):
            continue
        label_match = RECEPTION_LABEL_RE.search(chunk)
        if label_match and DATE_TOKEN_RE.search(chunk):
            add_candidate(
                label_match,
                chunk,
                chunk[label_match.end():label_match.end() + 8],
            )

    if not specific_candidates and not generic_candidates and RECEPTION_LABEL_RE.search(source):
    ...
    for candidate in specific_candidates + generic_candidates:
