...
from Crawler import municipal_dongducheon as ddc


@dataclass
class Target:
    provider: str = ddc.DONGDUCHEON_PROVIDER
    url: str = ddc.DONGDUCHEON_URL
    branch: str = "동두천시"
...
def _item(
    ...
    *,
    ...
    return {
        ...
        "d_co_sprvsn_id": ddc.DONGDUCHEON_CO_SPONSOR_ID,
        "d_edu_gvmnfc": ddc.DONGDUCHEON_BRANCH,
        "d_rgn": region,
...
class Backend:
    ...
    def landing(self) -> str:
        return f"""
        <html><head><title>동두천시 평생학습포털</title></head><body>
          <form><input id="s_resion_cd1" name="s_resion_cd1"
                       value="{ddc.DONGDUCHEON_REGION_CODE}">
          <input name="ARK_CO_SPRVSN_ID"
                 value="{ddc.DONGDUCHEON_CO_SPONSOR_ID}"></form>
          <p>총 <span>{len(self.rows)}</span>개의 강좌가 있습니다.</p>
          <script>const endpoint = '{ddc.DONGDUCHEON_LIST_PATH}/search';</script>
        </body></html>
        """
...
class FakeSession:
    ...
    def get(self, url: str, **kwargs: Any) -> FakeResponse:
        ...
        if url == ddc.DONGDUCHEON_URL:
            return FakeResponse(url=url, text=self.backend.landing())
        parsed = urlparse(url)
        assert parsed.scheme == "https"
        assert parsed.netloc == ddc.DONGDUCHEON_HOST
        assert parsed.path == ddc.DONGDUCHEON_DETAIL_PATH
        ...
        venue = self.backend.venues.get(identity, f"경기 동두천시 중앙로 110-18 합성강의실 {subject}")
        return FakeResponse(
            url=url,
            text=_detail_html(
                item,
    ...
    def post(self, url: str, **kwargs: Any) -> FakeResponse:
        ...
        assert url == ddc.DONGDUCHEON_API_URL
        data = kwargs["data"]
        assert set(data) == {"s_sort_by", "s_row_start", "s_row_end"}
        assert data["s_sort_by"] == "1"
        start = int(data["s_row_start"])
        end = int(data["s_row_end"])
        assert end - start == ddc.DONGDUCHEON_PAGE_SIZE
...
def _collect(
    ...
    **kwargs: Any,
    ...
    return ddc.collect_dongducheon_education_courses(
        Target(),
...
def test_target_urls_ranges_and_budget_constants_are_strict() -> None:
    assert ddc.DONGDUCHEON_PROVIDER == "MUNI_DDC_GSEEK_KR_97F9673C"
    assert ddc.DONGDUCHEON_CANDIDATE_ID == "MUNI_IR_56EC8BC6C16E"
    assert ddc.DONGDUCHEON_CO_SPONSOR_ID == "G000016"
    assert ddc.DONGDUCHEON_PARENT_PROVIDER == "GYEONGGI_GSEEK"
    assert ddc.DONGDUCHEON_PARENT_URL == ("https://www.gseek.kr/user/course/offline/list")
    assert ddc.DONGDUCHEON_PAGE_SIZE == 9
    assert ddc.DONGDUCHEON_DEFAULT_MAX_PAGES >= 18
    assert ddc.DONGDUCHEON_DEFAULT_DETAIL_LIMIT >= 138
    assert ddc.DONGDUCHEON_DEFAULT_MAX_REQUESTS >= 318
    assert ddc.is_dongducheon_education_target(Target())
    assert not ddc.is_dongducheon_education_target(Target(provider="WRONG"))
    for value in (
        "http://ddc.gseek.kr/user/course/offline/list",
        "https://ddc.gseek.kr:443/user/course/offline/list",
        ddc.DONGDUCHEON_URL + "?page=1",
        ddc.DONGDUCHEON_URL + "#fragment",
        ddc.DONGDUCHEON_PARENT_URL,
        "https://ddc.gseek.kr/user/course/offline/list/../view",
    ):
        assert not ddc.is_dongducheon_education_target(Target(url=value))
    assert ddc.dongducheon_api_range(1) == (1, 10)
    assert ddc.dongducheon_api_range(17) == (145, 154)
    assert ddc.dongducheon_api_range("../1") == (0, 0)
    assert ddc.dongducheon_sentinel_range(147) == (148, 157)
    assert ddc.dongducheon_sentinel_range("bad") == (0, 0)
    assert ddc.dongducheon_detail_url("61347", "1").endswith("s_sbjct_sn=61347&s_sbjct_cycl_sn=1")
    assert ddc.dongducheon_detail_url("61347&admin=1", "1") == ""
...
def test_complete_post_census_details_application_and_pii_minimization() -> None:
    ...
    assert parser == ddc.DONGDUCHEON_PARSER
    assert len(rows) == 2
    ...
    assert meta["source_region_counts"] == {"송내동": 9, "동두천시": 1}
    assert meta["current_source_region_counts"] == {"송내동": 1, "동두천시": 1}
    assert meta["branch_counts"] == {ddc.DONGDUCHEON_BRANCH: 2}
    ...
    assert meta["parent_aggregate_exclusion_value"] == "G000016"
    ...
    assert opened["provider"] == ddc.DONGDUCHEON_PROVIDER
    assert opened["provider_course_id"].endswith(":course:1001:1")
    assert opened["branch"] == ddc.DONGDUCHEON_BRANCH
    assert opened["region"] == ddc.DONGDUCHEON_MUNICIPALITY_NAME
...
def test_transient_failure_retries_with_a_new_managed_session() -> None:
    backend = Backend()
    backend.fail_once[("GET", ddc.DONGDUCHEON_URL)] = 1
    rows, _, meta = _collect(backend)
    assert len(rows) == 2
    assert meta["snapshot_complete"] is True
...
def test_duplicate_identity_test_notice_and_semantic_duplicate_fail_closed() -> None:
    ...
    semantic.venues["1001:1"] = "동두천시 동일 강의실"
    semantic.venues["1002:1"] = "동두천시 동일 강의실"
    rows, _, meta = _collect(semantic)
    assert rows == []
    assert meta["semantic_duplicate_count"] == 1


def test_owner_boundary_rejects_any_non_g000016_source_row() -> None:
    backend = Backend()
    backend.rows[-1]["d_co_sprvsn_id"] = "G000001"
    rows, _, meta = _collect(backend)
    assert rows == []
    assert meta["snapshot_complete"] is False
    assert "non-Dongducheon site supervision" in meta["configured_collection_error"]
    assert meta["ownership_co_sponsor_id"] == "G000016"
    assert meta["parent_aggregate_exclusion_required"] is True


def test_managed_session_and_external_dedupe_are_fail_closed() -> None:
    rows, _, meta = ddc.collect_dongducheon_education_courses(Target())
    assert rows == []
    assert "managed session_factory" in meta["configured_collection_error"]

    rows, _, meta = ddc.collect_dongducheon_education_courses(
        Target(provider="WRONG"),
        session_factory=Backend().session_factory,
    )
    assert rows == []
    assert "canonical Dongducheon" in meta["configured_collection_error"]
...
@pytest.mark.skipif(
    os.environ.get("RUN_LIVE_DONGDUCHEON") != "1",
    reason="set RUN_LIVE_DONGDUCHEON=1 for the exact 2026-07-22 live audit",
)
def test_exact_live_dongducheon_snapshot_20260722() -> None:
    rows, parser, meta = ddc.collect_dongducheon_education_courses(
        Target(),
        timeout=40,
        max_pages=30,
        detail_limit=250,
    ...
    assert meta["pagination_complete"] is True
    assert meta["details_complete"] is True
    assert meta["source_total"] == meta["source_rows"] == 147
    assert meta["data_pages"] == 17
    assert meta["pages"] == 18
    assert meta["list_requests"] == 20
    assert meta["page_counts"] == {
        **{page: 9 for page in range(1, 17)},
        17: 3,
        18: 0,
    }
    assert meta["sentinel_start"] == 148
    assert meta["sentinel_end"] == 157
    assert meta["sentinel_count"] == 0
    assert meta["stability_rechecks"] == 2
    assert meta["first_identity"] == "61347:1"
    assert meta["last_identity"] == "58236:1"
    assert len(meta["first_page_signature"]) == 64
    assert len(meta["last_page_signature"]) == 64
    assert meta["expired_count"] == 9
    assert meta["current_count"] == meta["returned_count"] == len(rows) == 138
    assert meta["detail_attempts"] == meta["detail_pages"] == 138
    assert meta["detail_errors"] == 0
    ...
    assert meta["status_counts"] == {"CLOSED": 74, "SCHEDULED": 59, "OPEN": 5}
    assert meta["source_region_counts"] == {"송내동": 146, "동두천시": 1}
    assert meta["current_source_region_counts"] == {"송내동": 137, "동두천시": 1}
    assert meta["branch_counts"] == {ddc.DONGDUCHEON_BRANCH: 138}
    assert meta["branch_count"] == 1
    assert meta["venue_count"] == 18
    assert meta["duplicate_count"] == 0
    assert meta["duplicate_url_count"] == 0
    assert meta["semantic_duplicate_count"] == 0
    assert meta["test_or_notice_row_count"] == 0
    assert meta["pii_redaction_count"] == 0
    assert meta["application_control_count"] == 138
    assert meta["reservation_discovery_links"] == 5
    assert meta["required_logical_requests"] == 159
    assert meta["physical_requests"] == 159
    assert meta["retry_count"] == 0
    assert meta["sessions_created"] == 2
    assert meta["application_endpoints_called"] == 0
    assert all(row["provider"] == ddc.DONGDUCHEON_PROVIDER for row in rows)
    assert all(row["branch"] == ddc.DONGDUCHEON_BRANCH for row in rows)
    assert all(row["region"] == ddc.DONGDUCHEON_MUNICIPALITY_NAME for row in rows)
    assert all(row.get("venue_name") for row in rows)
    assert sum(bool(row.get("application_url")) for row in rows) == 5


@pytest.mark.skipif(
    os.environ.get("RUN_LIVE_DONGDUCHEON") != "1",
    reason="set RUN_LIVE_DONGDUCHEON=1 for the parent-owner overlap audit",
)
def test_exact_live_parent_aggregate_owner_boundary_20260722() -> None:
    ...
    def census(url: str, *, region: bool) -> list[dict[str, Any]]:
        ...
        for page in range(1, 18):
            start, end = ddc.dongducheon_api_range(page)
            data = {
                "s_sort_by": "1",
            ...
            if region:
                data["resion"] = ddc.DONGDUCHEON_REGION_CODE
    ...
    branded = census(ddc.DONGDUCHEON_API_URL, region=False)
    parent_filtered = census(ddc.DONGDUCHEON_PARENT_API_URL, region=True)
    first_parent = session.post(
        ddc.DONGDUCHEON_PARENT_API_URL,
        data={"s_sort_by": "1", "s_row_start": "1", "s_row_end": "10"},
        headers={"Referer": ddc.DONGDUCHEON_PARENT_URL},
        timeout=40,
        allow_redirects=False,
    ).json()
    ...
    assert len(branded) == len(branded_ids) == 147
    assert len(parent_filtered) == len(filtered_ids) == 147
    assert branded_ids == filtered_ids
    assert branded_ids.isdisjoint(first_ids)
    assert all(item["d_co_sprvsn_id"] == ddc.DONGDUCHEON_CO_SPONSOR_ID for item in parent_filtered)
    assert {item["d_total_cnt"] for item in parent_filtered} == {"147"}
    assert {item["d_rgn"] for item in parent_filtered} == {"동두천시"}
    assert [identity(item) for item in branded] == [identity(item) for item in parent_filtered]
